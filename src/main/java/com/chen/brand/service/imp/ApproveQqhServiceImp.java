...
public class ApproveQqhServiceImp implements ApproveQqhService{
    ...
    public int[] total(Long userId){
        int[] ans = new int[2];
        ans[0] = qqhMapper.count(null, null, null, userId);
        ans[1] = qqhMapper.count(null, null, ApproveStatus.FinalApprovePass.getStatus(), userId);
        return ans;
    }

    public List<ApproveQqh> findByUserIdAndStatus(Long userId, Long status){
        return qqhMapper.findByUserIdAndStatus(userId, status);
    }
}
