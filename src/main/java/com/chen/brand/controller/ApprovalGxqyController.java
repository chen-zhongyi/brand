...
import com.chen.brand.http.request.ApprovalGxqy.GxqyRequest;
...
@RestController
@RequestMapping( value = "/api/gxqys")
@Api( value = "api --- ApprovalGxqyController", description = "高薪技术企业认证")
public class ApprovalGxqyController extends BaseController{

    @Autowired
    private ApprovalGxqyService jnzlService;
    ...
    @PostMapping(value = "")
    public Map<String, Object> insert(@ApiIgnore HttpServletRequest httpRequest, @RequestBody @Valid GxqyRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
        ApprovalGxqy jnzl = new ApprovalGxqy();
        jnzl.setJb(request.getJb());
        jnzl.setSj(request.getSj());
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "request", value = "请求体", dataType = "gxqyRequest", paramType = "body")
    })
    @PutMapping(value = "/{id}")
    public Map<String, Object> update(@PathVariable Long id, @RequestBody @Valid GxqyRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
