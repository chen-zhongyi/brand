...
import com.chen.brand.http.request.ApproveJwxc.ApproveJwxcRequest;
...
@RestController
@RequestMapping( value = "/api/Jwxc")
@Api( value = "api --- ApproveJwxcController", description = "境外宣传媒介")
public class ApproveJwxcController extends BaseController{

    @Autowired
    private ApproveJwxcService bzrzService;

    @ApiOperation(value = "创建")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "request", value = "请求体", dataType = "ApproveJwxcRequest", paramType = "body")
    })
    @PostMapping(value = "")
    public Map<String, Object> insert(@ApiIgnore HttpServletRequest httpRequest, @RequestBody @Valid ApproveJwxcRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
        ApproveJwxc bzrz = new ApproveJwxc();
        bzrz.setLx(request.getLx());
        bzrz.setSm(request.getSm());
    ...
    @PutMapping(value = "/{id}")
    public Map<String, Object> update(@PathVariable Long id, @RequestBody @Valid ApproveJwxcRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
