...
@RestController
@RequestMapping( value = "/api/bzrz3s")
@Api( value = "api --- ApproveBzrz3Controller", description = "产品或生产线认证")
public class ApproveBzrz3Controller extends BaseController{

    @Autowired
    private ApproveBzrz3Service bzrzService;
    ...
    @PostMapping(value = "")
    public Map<String, Object> insert(@ApiIgnore HttpServletRequest httpRequest, @RequestBody @Valid ApproveBzrzRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
        ApproveBzrz3 bzrz = new ApproveBzrz3();
        bzrz.setBz(request.getBz());
    ...
    @PutMapping(value = "/{id}")
    public Map<String, Object> update(@PathVariable Long id, @RequestBody @Valid ApproveBzrzRequest request, @ApiIgnore Errors errors) {
        if (errors.hasErrors()) {
            return createResponse(Constant.FAIL, "参数验证失败", null);
        }
